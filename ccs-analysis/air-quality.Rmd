...
clean_multi_eval_fisher <- function(dataframe) {
  ...
  group_by(question) %>%
  summarise(
    Least_with = n[vulnerable == "Least"],
    Most_with = n[vulnerable == "Most"],
    Least_without = total[vulnerable == "Least"] - n[vulnerable == "Least"],
    Most_without = total[vulnerable == "Most"] - n[vulnerable == "Most"]
  ) %>%
  rowwise() %>%
  do({
    tbl <- matrix(c(.$Least_with, .$Most_with, .$Least_without, .$Most_without), nrow = 2)
    test_result <- fisher.test(tbl)
    data.frame(., Odds_Ratio = test_result$estimate["odds ratio"], P_Value = test_result$p.value)
  })
}
...
data_tracts <- c(unique(air_survey$census_tract_full))
data_blocks <- c(unique(air_survey$census_block_full))

# https://www.atsdr.cdc.gov/placeandhealth/svi/index.html
#vi_tract <- read.csv("/Volumes/cbjackson2/ccs-knowledge/census-data/svi/svi-wi-tract.csv")
#svi_response <- svi_tract[which(svi_tract$FIPS %in% data_tracts),]

#svi_eval <- svi_tract[which(svi_tract$COUNTY == c("Dane","Jefferson")),]
# Calculate quartiles
#Q1 <- quantile(svi_eval$RPL_THEMES, 0.25)
#Q3 <- quantile(svi_eval$RPL_THEMES, 0.75)

#svi_response$vulnerable <- ifelse(svi_response$RPL_THEMES > Q3, "Most","Least")
#svi_response$FIPS <- as.character(svi_response$FIPS)

# Which variables are important to include in the analysis?

#https://www.atsdr.cdc.gov/placeandhealth/eji/indicators.html
<<<<<<< Updated upstream
# https://www.dhs.wisconsin.gov/epht/environmental-justice.htm 
...
#eji <- read.csv("//Volumes/cbjackson2/ccs-knowledge/census-data/environmental/eji-wisconsin.csv")
#eji <- eji[which(!is.na(eji$RPL_EBM)),]
#eji_eval <- eji[which(eji$COUNTY == c("Dane","Jefferson")),]
>>>>>>> Stashed changes

# Calculate quartiles
#Q1 <- quantile(eji_eval$RPL_EBM, 0.25)
#Q3 <- quantile(eji_eval$RPL_EBM, 0.75)

#eji_response <- eji[which(eji$geoid %in% data_tracts),]
eji$vulnerable <- ifelse(eji$RPL_EBM > Q3, "Most","Least")
eji_response_analysis <- eji %>%
...
wisconsin_tract <- st_read("/Volumes/cbjackson2/ccs-knowledge/ccs-data/geography/tl_2022_55_tract/tl_2022_55_tract.shp")
...
air_survey$census_tract_full <- as.character(air_survey$census_tract_full) # CHANGE NAME
# Group people into least/most vulnerable areas 
<<<<<<< Updated upstream
air_survey <- left_join(air_survey, eji_response_analysis, by =c("census_tract_full"="geoid")) 

# ISSUE HERE WITH JOINING. MANY ITEMS NOT INCLUDED
=======
#air_survey <- left_join(air_survey, svi_response, by =c("census_tract_full"="geoid")) # CHANGE NAME
eji_response_analysis$geoid <- as.character(eji_response_analysis$geoid )
air_survey <- left_join(air_survey, eji_response_analysis, by =c("census_tract_full"="geoid")) # CHANGE NAME
>>>>>>> Stashed changes
...
vulnerable_q2_fisher <- clean_multi_eval_fisher(vulnerable_q2)
...
vulnerable_q3_chi <- clean_multi_eval_chi(vulnerable_q3)
vulnerable_q3_fisher <- clean_multi_eval_fisher(vulnerable_q3)
...
vulnerable_q4_fisher <- clean_multi_eval_fisher(vulnerable_q4)
...
vulnerable_q6_fisher <- clean_multi_eval_fisher(vulnerable_q6)
...
vulnerable_q7_fisher <- clean_multi_eval_fisher(vulnerable_q7)

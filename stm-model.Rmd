...
# Fit the topic model
num_topics <- 3  # choose an appropriate number of topics
topic_model <- stm(documents=docs, vocab=vocab, K=num_topics, data=meta, max.em.its=75, init.type="Spectral")

# Visualize the model
plot(topic_model, type="summary", main="Topic Model Summary")
...
subset_df <- df %>%
  filter(edu_recode %in% filtered_categories, !is.na(edu_recode)) 
...
perform_topic_modeling(subset_df, "edu_recode")
```

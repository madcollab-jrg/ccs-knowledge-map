...
get_demographic_info = function(head, surveys, save_dir, save_name){
  save_loc = "/Volumes/cbjackson2/ccs-knowledge/"
  for(i in 1:length(surveys)){
    survey_dir = paste(head, surveys[i], sep = "")
    survey_files = get_files(survey_dir) # get all surveys from the survey directory;
    
    # combine the demographic data
    combined_data = combine_files(survey_dir, survey_files[[1]], type = "demo")
    ...
    save_dir_survey_dir = paste(save_dir_head,surveys[i],sep="")
    if(!str_sub(surveys[i],1,-2) %in% list.files(save_dir_head)){
...
get_demographic_info(head, surveys, save_dir, survey_save_name)
...
save_dir = "ccs-data-demographic_unprocessed"
survey_save_name = "demographic_data.csv"
get_demographic_info(head, surveys, save_dir, survey_save_names)
